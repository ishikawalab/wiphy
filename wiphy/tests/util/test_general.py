...
class Test(unittest.TestCase):
    ...
    def test_frodiff(self):
        fro = me.frodiff(xp.array([1, 1j, 0, 0]), xp.array([1, 0, 1j, 0]))
    ...
    def test_getEuclideanDistances(self):
        ...
        ret = me.asnumpy(me.getEuclideanDistances(xp.array(codes)))
    ...
    def test_getMinimumEuclideanDistance(self):
        codes = mod.generatePSKSymbols(4).reshape(4, 1, 1)
        med = me.getMinimumEuclideanDistance(xp.array(codes))
    ...
    def test_getDFTMatrix(self):
        ...
        W = me.getDFTMatrix(8)
        np.testing.assert_almost_equal(W.dot(W.conj().T), xp.eye(8, dtype=xp.complex), decimal=3)
        W = me.getDFTMatrix(16)
        np.testing.assert_almost_equal(W.dot(W.conj().T), xp.eye(16, dtype=xp.complex), decimal=3)
    ...
    def test_getXORtoErrorBitsArray(self):
        ...
        np.testing.assert_almost_equal(a, xp.array([0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2, 3, 2, 3, 3, 4, 1]))

    def test_getErrorBitsTable(self):
        t = me.getErrorBitsTable(4)
        np.testing.assert_almost_equal(t, xp.array([[0, 1, 1, 2], [1, 0, 2, 1], [1, 2, 0, 1], [2, 1, 1, 0]]))
        t = me.getErrorBitsTable(8)
        np.testing.assert_almost_equal(t, xp.array(
